...
/*
    ...
    */
    selectors.insert(".SelectMenu-modal:::relative inset-0 z-98 flex max-h-0[66%] my-1 mx-0 overflow-hidden pointer-events-auto flex-col bg-color-gray-50 border border-solid border-slate-400 rounded-md shadow-md shadow-slate-400 w-fit".to_owned()); //  animation[SelectMenu-modal-animation&0.12s&cubic-bezier(0,&0.1,&0.1,&1)&backwards;@keyframes&SelectMenu-modal-animation&{0%&{opacity:&0;transform:&scale(0.9);}}]
