...
pub(crate) fn details_selectors() -> Vec<String> {
    ...
    selectors.push(".SelectMenu:::w-fit relative inset-0 z-98 flex p-1 pointer-events-none flex-col".to_owned());
    selectors.push(".SelectMenu::before:::absolute inset-0 z-98 flex p-4 pointer-events-none content-[\"&\"]".to_owned());
    selectors.push(".SelectMenu-modal:::relative inset-0 z-98 flex max-h-0[66%] my-1 mx-0 overflow-hidden pointer-events-auto flex-col bg-color-gray-50 border border-solid border-slate-400 rounded-md shadow-sm w-fit".to_owned()); //  animation[SelectMenu-modal-animation&0.12s&cubic-bezier(0,&0.1,&0.1,&1)&backwards;@keyframes&SelectMenu-modal-animation&{0%&{opacity:&0;transform:&scale(0.9);}}]
    selectors.push(".SelectMenu-list:::flex flex-col p-0 mt-0 mr-0 ml-0 mb-0[-1px] flex-auto overflow-x-hidden overflow-y-auto bg-color-gray-50".to_owned());
    selectors.push(".SelectMenu-item:::flex items-center w-full px-4 py-1 overflow-hidden text-left cursor-pointer border-t-0 border-l-0 border-r-0 border-b border-solid border-gray-400".to_owned());
