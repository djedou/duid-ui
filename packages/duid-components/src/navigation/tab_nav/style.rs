...
pub(crate) fn get_tab_nav_selectors() -> HashSet<String> {
    ...
    let _ = selectors.insert(".tabnav-tab:::inline-block shrink-0 py-2 px-4 text-xs color-gray-800 no-underline bg-color-transparent rounded-tl-md rounded-tr-md border border-solid border-transparent border-b-0 transition-none[color] duration-75[0.2s] ease-in[cubic-bezier(0.3,&0,&0.5,&1)]".to_owned());
